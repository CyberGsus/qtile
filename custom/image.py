...
from custom.utils import update_tag
from custom.utils import update_style_tag
...
formatter = ColorFormatter(
    "$RESET$COLOR%(asctime)s  $RESET$BOLD[$COLOR%(filename)s:%(funcName)s()"
    + ":%(lineno)d$RESET$BOLD] $COLOR$BOLD%(levelname)s$RESET %(message)s"
)
...
try:
    ...
    # TODO: add ids to svg and edit tags through ids instead of
    #       directly indexing the lines
    def process_image(pattern, name, height, width):
        ...
        # Update dimensions
        pat[1] = update_tag(pat[1], {"height": height, "width": width})
        ...
        pat = "\n".join(pat)
        logger.info(
            "Exporting image to \x1b[38;5;159m{}".format(
                os.path.join(img_path, name + ".png")
            )
        )

        svg2png(bytestring=pat, write_to=os.path.join(img[name]))
